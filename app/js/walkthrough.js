...
class WalkthroughManager {
  ...
  init() {
    ...
    const self = this;
    Shiny.addCustomMessageHandler('updateWalkthroughStep', function(data) {
      self.showStep(data);
    });

    Shiny.addCustomMessageHandler('hideWalkthrough', function(data) {
      self.hide();
    });
  ...
  removeDefaultTooltips() {
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      elementsWithTitle.forEach(el => {
        if (el.getAttribute('title') && el.getAttribute('title').includes('<')) {
      ...
      mutations.forEach((mutation) => {
        if (mutation.type === 'childList') {
          mutation.addedNodes.forEach((node) => {
            if (node.nodeType === 1) {
              const elementsWithTitle = node.querySelectorAll ? node.querySelectorAll('[title]') : [];
              elementsWithTitle.forEach(el => {
                if (el.getAttribute('title') && el.getAttribute('title').includes('<')) {
                  el.removeAttribute('title');
                }
              });
              if (node.getAttribute && node.getAttribute('title') && node.getAttribute('title').includes('<')) {
